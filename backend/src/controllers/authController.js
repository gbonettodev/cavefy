...
const tokenDe = (usuario) =>
  jwt.sign({ id: usuario.id, papel: usuario.papel }, env.jwtSecret, {
    expiresIn: "7d",
  });
  ...
  try {
    const { rows } = await pool.query(
      `
        ...
        VALUES ($1, $2, $3)
        RETURNING id, nome, email, papel, foto_url
      `,
      [dados.nome, dados.email.toLowerCase(), senhaHash],
    );
...
export async function login(req, res) {
  ...
  const { rows } = await pool.query(
    `
      SELECT id, nome, email, papel, foto_url, senha_hash
      FROM usuarios
      WHERE email = $1
    `,
...
export async function atualizarPerfil(req, res) {
  ...
  try {
    ...
    const foto = fileReference(req.file);
    const senhaHash = dados.senha?.trim() ? await bcrypt.hash(dados.senha, 10) : null;
    const { rows } = await pool.query(
      `
        UPDATE usuarios
        SET
        ...
        RETURNING id, nome, email, papel, foto_url
      `,
      [dados.nome, email, senhaHash, foto, req.usuario.id],
    );
    persisted = true;
    const usuario = rows[0];

    if (foto) await removeStoredFile(req.usuario.foto_url);
    return res.json({ usuario, token: tokenDe(usuario) });
  } catch (error) {
