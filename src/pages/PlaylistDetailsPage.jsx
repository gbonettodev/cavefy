import { useEffect, useState } from "react";
import { useNavigate, useOutletContext, useParams } from "react-router-dom";
import { toast } from "react-toastify";
import { ArrowLeft, Disc3, ListMusic, Play, X } from "lucide-react";
import { apiFetch } from "../services/api";
import { duration, mediaUrl } from "../services/media";
import { useCavefyStore } from "../store/index";
import Cover from "../components/Cover";
export default function PlaylistDetailsPage() {
  const { id } = useParams();
  const { tocar, recarregarPlaylists } = useOutletContext();
  const token = useCavefyStore((state) => state.token);
  const navigate = useNavigate();
  const [playlist, setPlaylist] = useState(null);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    let active = true;
    async function load() {
      setLoading(true);
      try {
        const result = await apiFetch(`/playlists/${id}`, {}, token);
        if (active) setPlaylist(result);
      } catch (error) {
        if (active) {
          toast.error(error.message);
          setPlaylist(null);
        }
      } finally {
        if (active) setLoading(false);
      }
    }
    load();
    return () => {
      active = false;
    };
  }, [id, token]);
  async function remover(song) {
    try {
      await apiFetch(
        `/playlists/${id}/musicas/${song.id}`,
        { method: "DELETE" },
        token,
      );
      await recarregarPlaylists();
      setPlaylist((current) =>
        current
          ? {
              ...current,
              musicas: current.musicas.filter(
                (item) => String(item.id) !== String(song.id),
              ),
              total_musicas: Math.max(0, current.total_musicas - 1),
            }
          : current,
      );
      toast.success("Música removida da playlist.");
    } catch (error) {
      toast.error(error.message);
    }
  }
  if (loading)
    return (
      <div className="empty-state">
        <Disc3 className="spin" size={30} />
        <p>Abrindo playlist...</p>
      </div>
    );
  if (!playlist)
    return (
      <div className="empty-state">
        <ListMusic size={30} />
        <h3>Playlist não encontrada</h3>
        <button
          className="text-button"
          type="button"
          onClick={() => navigate("/playlists")}
        >
          Voltar para playlists
        </button>
      </div>
    );
  const playable = playlist.musicas.find((song) => song.audio_url);
  return (
    <div className="playlist-details-page">
      <button
        className="back-link"
        type="button"
        onClick={() => navigate("/playlists")}
      >
        <ArrowLeft size={17} /> Voltar para playlists
      </button>
      <section className="playlist-detail-hero">
        <div className={`playlist-detail-art playlist-${playlist.id % 4}`}>
          {playlist.capa_url ? (
            <img
              className="playlist-detail-art-image"
              src={mediaUrl(playlist.capa_url)}
              alt={`Capa da playlist ${playlist.nome}`}
            />
          ) : (
            <ListMusic size={72} />
          )}
        </div>
        <div>
          <span className="eyebrow">
            Sua biblioteca · {playlist.total_musicas || 0} músicas
          </span>
          <h1>{playlist.nome}</h1>
          <p>{playlist.descricao || "Uma seleção feita por você."}</p>
          <button
            className="button button-gold"
            type="button"
            disabled={!playable}
            onClick={() => tocar(playable, playlist.musicas)}
          >
            <Play size={17} fill="currentColor" />{" "}
            {playable ? "Reproduzir playlist" : "Sem áudio disponível"}
          </button>
        </div>
      </section>
      <div className="playlist-songs-heading">
        <div>
          <span className="eyebrow">Faixas da coleção</span>
          <h2>Todas as músicas</h2>
        </div>
        <span>
          {playlist.musicas.length}{" "}
          {playlist.musicas.length === 1 ? "música" : "músicas"}
        </span>
      </div>
      {playlist.musicas.length ? (
        <div className="catalog-list playlist-songs-list">
          {playlist.musicas.map((song, index) => (
            <div className="song-row" key={song.id}>
              <span className="row-number">{String(index + 1).padStart(2, "0")}</span>
              <Cover song={song} size="tiny" />
              <div className="row-info">
                <strong>{song.titulo}</strong>
                <span>{song.artista}</span>
              </div>
              <span className="row-genre">{song.genero || "—"}</span>
              <span className="row-duration">{duration(song.duracao_segundos)}</span>
              <button
                type="button"
                aria-label={
                  song.audio_url
                    ? `Reproduzir ${song.titulo}`
                    : `${song.titulo} está sem áudio`
                }
                onClick={() =>
                  song.audio_url
                    ? tocar(song, playlist.musicas)
                    : toast.info("Esta música ainda não possui áudio.")
                }
              >
                <Play size={15} fill="currentColor" />
              </button>
              <button
                className="playlist-song-remove"
                type="button"
                aria-label={`Remover ${song.titulo} da playlist`}
                onClick={() => remover(song)}
              >
                <X size={15} />
              </button>
            </div>
          ))}
        </div>
      ) : (
        <div className="empty-state">
          <ListMusic size={30} />
          <h3>Esta playlist está vazia</h3>
          <p>Adicione músicas pelo botão de playlist na página de cada faixa.</p>
        </div>
      )}
    </div>
  );
}
