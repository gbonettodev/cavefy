import { useMemo, useState } from "react";
import { useNavigate, useOutletContext } from "react-router-dom";
import { ChevronDown, Disc3, Plus, Search } from "lucide-react";
import PageIntro from "../components/PageIntro";
import SongRow from "../components/SongRow";
export default function CatalogPage() {
  const { musicas, generos, tocar, carregando } = useOutletContext();
  const navigate = useNavigate();
  const [search, setSearch] = useState("");
  const [genre, setGenre] = useState("todos");
  const filtered = useMemo(
    () =>
      musicas.filter(
        (song) =>
          `${song.titulo} ${song.artista} ${song.album}`
            .toLowerCase()
            .includes(search.toLowerCase()) &&
          (genre === "todos" || String(song.genero_id) === genre),
      ),
    [musicas, search, genre],
  );
  return (
    <>
      <PageIntro
        eyebrow="Explorar"
        title="Catálogo musical"
        subtitle="Encontre uma faixa para cada capítulo da sua história."
        action={
          <button
            className="button button-outline"
            onClick={() => navigate("/musicas/novo")}
          >
            <Plus size={17} /> Nova música
          </button>
        }
      />
      <div className="catalog-tools">
        <label className="search-field">
          <Search size={18} />
          <input
            value={search}
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Buscar por música, artista ou álbum"
          />
        </label>
        <label className="select-field">
          <span>Gênero</span>
          <select value={genre} onChange={(event) => setGenre(event.target.value)}>
            <option value="todos">Todos os gêneros</option>
            {generos.map((item) => (
              <option key={item.id} value={item.id}>
                {item.nome}
              </option>
            ))}
          </select>
          <ChevronDown size={16} />
        </label>
      </div>
      {carregando ? (
        <div className="empty-state">
          <Disc3 className="spin" size={30} />
          <p>Consultando as cavernas...</p>
        </div>
      ) : filtered.length ? (
        <div className="catalog-list">
          {filtered.map((song, index) => (
            <SongRow
              key={song.id}
              song={song}
              index={index + 1}
              onPlay={() => tocar(song, musicas)}
              onOpen={() => navigate(`/musicas/${song.id}`)}
            />
          ))}
        </div>
      ) : (
        <div className="empty-state">
          <Search size={30} />
          <h3>Nenhuma faixa encontrada</h3>
          <p>Tente outro termo ou gênero.</p>
        </div>
      )}
    </>
  );
}
