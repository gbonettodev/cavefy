...
export default function AuthScreen({ cadastro = false }) {
  ...
  const form = useForm({
    resolver: zodResolver(modoCadastro ? cadastroSchema : authSchema),
    ...
    try {
      ...
      toast.error(
        error.message.includes("fetch")
          ? "API offline. Inicie o backend para continuar."
          : error.message,
      );
    } finally {
      setEnviando(false);
    ...
    <main className="auth-screen">
      ...
      <section className="auth-card">
        ...
        <p className="auth-subtitle">
          {modoCadastro
            ? "Crie seu espaço para catalogar, descobrir e montar playlists."
            : "Sua biblioteca sonora, do seu jeito."}
        </p>
        ...
        <div className="auth-switch">
          ...
          <button
            onClick={() => navigate(modoCadastro ? "/login" : "/cadastro")}
            type="button"
          >
      ...
      <p className="auth-footer">
        CAVEFY © 2026 · Feito para quem escuta além do óbvio.
      </p>
    </main>
  );
}
