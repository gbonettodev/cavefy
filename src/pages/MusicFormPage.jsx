import { useEffect, useState } from "react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { toast } from "react-toastify";
import { useNavigate, useOutletContext, useParams } from "react-router-dom";
import {
  Album,
  ArrowLeft,
  ArrowRight,
  FileAudio,
  Sparkles,
  Upload,
} from "lucide-react";
import Field from "../components/Field";
import PageIntro from "../components/PageIntro";
import { musicaSchema } from "../validation/schemas";
export default function MusicFormPage({ editar = false }) {
  const { id } = useParams();
  const { musicas, generos, salvarMusica } = useOutletContext();
  const navigate = useNavigate();
  const current = editar
    ? musicas.find((item) => String(item.id) === id)
    : null;
  const [saving, setSaving] = useState(false);
  const [coverPreview, setCoverPreview] = useState(current?.capa_url || "");
  const [audioName, setAudioName] = useState("");
  const form = useForm({
    resolver: zodResolver(musicaSchema),
    defaultValues: current
      ? { ...current, genero_id: current.genero_id }
      : {
          titulo: "",
          artista: "",
          album: "",
          genero_id: "",
          ano: "",
          duracao_segundos: "",
          descricao: "",
        },
  });
  useEffect(() => {
    if (current) form.reset({ ...current, genero_id: current.genero_id });
  }, [current, form]);
  async function submit(data) {
    setSaving(true);
    try {
      const result = await salvarMusica(
        data,
        form.getValues("_files") || {},
        current?.id,
      );
      toast.success(
        editar ? "Música atualizada." : "Música adicionada ao catálogo.",
      );
      navigate(`/musicas/${result.id}`);
    } catch (error) {
      toast.error(error.message);
    } finally {
      setSaving(false);
    }
  }
  function files(event) {
    const selected = event.target.files?.[0];
    const field = event.target.name;
    if (!selected) return;
    const validAudio = /\.(mp3|wav|ogg)$/i.test(selected.name);
    if (
      field === "audio" &&
      (!validAudio || selected.size > 25 * 1024 * 1024)
    ) {
      toast.error(
        !validAudio
          ? "Escolha um arquivo MP3, WAV ou OGG."
          : "O áudio precisa ter no máximo 25 MB.",
      );
      event.target.value = "";
      return;
    }
    if (field === "capa" && selected.size > 5 * 1024 * 1024) {
      toast.error("A capa precisa ter no máximo 5 MB.");
      event.target.value = "";
      return;
    }
    form.setValue("_files", {
      ...(form.getValues("_files") || {}),
      [field]: event.target.files,
    });
    if (field === "capa") setCoverPreview(URL.createObjectURL(selected));
    if (field === "audio") setAudioName(selected.name);
  }
  return (
    <div className="form-page">
      <button className="back-link" onClick={() => navigate(-1)}>
        <ArrowLeft size={17} /> Cancelar
      </button>
      <PageIntro
        eyebrow={editar ? "Editar faixa" : "Nova faixa"}
        title={editar ? "Ajuste os detalhes." : "Catalogue um novo som."}
        subtitle="Preencha os dados para guardar essa descoberta no CAVEFY."
      />
      <form className="music-form" onSubmit={form.handleSubmit(submit)}>
        <div className="form-card">
          <div className="form-card-heading">
            <div className="step-number">01</div>
            <div>
              <h2>Identidade da faixa</h2>
              <p>Como ela será encontrada no seu catálogo.</p>
            </div>
          </div>
          <div className="form-grid">
            <Field
              label="Título da música"
              error={form.formState.errors.titulo?.message}
            >
              <input
                {...form.register("titulo")}
                placeholder="Ex.: Echoes in Stone"
              />
            </Field>
            <Field
              label="Artista"
              error={form.formState.errors.artista?.message}
            >
              <input
                {...form.register("artista")}
                placeholder="Ex.: The Stone Keepers"
              />
            </Field>
            <Field label="Álbum">
              <input
                {...form.register("album")}
                placeholder="Nome do álbum ou single"
              />
            </Field>
            <Field
              label="Gênero"
              error={form.formState.errors.genero_id?.message}
            >
              <select {...form.register("genero_id")}>
                <option value="">Escolha um gênero</option>
                {generos.map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.nome}
                  </option>
                ))}
              </select>
            </Field>
            <Field label="Ano de lançamento">
              <input
                type="number"
                {...form.register("ano")}
                placeholder="2026"
              />
            </Field>
            <Field label="Duração (segundos)">
              <input
                type="number"
                {...form.register("duracao_segundos")}
                placeholder="Ex.: 214"
              />
            </Field>
            <Field label="Descrição" className="wide">
              <textarea
                {...form.register("descricao")}
                placeholder="Conte um pouco sobre essa faixa..."
              />
            </Field>
          </div>
        </div>
        <div className="form-card">
          <div className="form-card-heading">
            <div className="step-number">02</div>
            <div>
              <h2>Textura e som</h2>
              <p>Adicione os arquivos criados por você.</p>
            </div>
          </div>
          <div className="upload-grid">
            <label className="upload-box cover-upload">
              <input
                type="file"
                name="capa"
                accept="image/png,image/jpeg,image/webp"
                onChange={files}
              />
              <div
                className={`upload-preview ${coverPreview ? "has-image" : ""}`}
                style={
                  coverPreview
                    ? { backgroundImage: `url(${coverPreview})` }
                    : {}
                }
              >
                {!coverPreview && <Album size={26} />}
              </div>
              <div>
                <strong>
                  {coverPreview ? "Trocar capa" : "Adicionar capa"}
                </strong>
                <span>PNG, JPG ou WEBP · até 5 MB</span>
              </div>
              <Upload size={18} />
            </label>
            <label className="upload-box">
              <input
                type="file"
                name="audio"
                accept="audio/mpeg,audio/mp3,audio/wav,audio/ogg"
                onChange={files}
              />
              <div className="upload-icon">
                <FileAudio size={24} />
              </div>
              <div>
                <strong>{audioName || "Adicionar áudio"}</strong>
                <span>
                  {audioName
                    ? "Arquivo pronto para envio"
                    : "MP3, WAV ou OGG · até 25 MB"}
                </span>
              </div>
              <Upload size={18} />
            </label>
          </div>
          <p className="helper-note">
            <Sparkles size={15} /> Faixas sem MP3 continuam visíveis no
            catálogo, mas não terão reprodução.
          </p>
        </div>
        <div className="form-actions">
          <button
            type="button"
            className="button button-ghost"
            onClick={() => navigate(-1)}
          >
            Cancelar
          </button>
          <button className="button button-gold" disabled={saving}>
            {saving
              ? "Salvando..."
              : editar
                ? "Salvar alterações"
                : "Adicionar ao CAVEFY"}{" "}
            <ArrowRight size={16} />
          </button>
        </div>
      </form>
    </div>
  );
}
